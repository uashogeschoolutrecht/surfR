---
title: "Intro to R"
author: "Marc A.T. Teunis; Jan-Willem Lankhaar"
date: "11/02/2018"
output:
...
Spent 4 minutes explaining to your neighbour:
 ...
 - What type of problems are you planning to solve
...
df$var_k[df$var_k == -999] <- NA
df$var_m[df$var_m ==  999] <- NA
df$var_n[df$var_n == -999] <- NA
df$var_o[df$var_n == -988] <- "NA"
## there are at least 5 errors in this code
```
 
__Reference Example (adopted from): Hadley Wickham, "Expressing yourself with R" Lecture  - https://richmedia.lse.ac.uk/methodologyinstitute/20170814_ExpressYourselfUsingR.mp4__ 
...
## Writing a function that replaces `-999` for `NA`
```{r}
...
```{r}
df <- purrr::modify_if(df, is.numeric, replace_999_for_na)
df
```
 ...
 * 'Input-in is input-out' (to make the function 'pipe-pable')
 
_You will hear about the 'pipe (`%>%`)' later_ 
...
```{r}
set.seed(1234)
normals <- rnorm(n = 100000, mean = 5, sd = 1.3)
...
## change number of observations to 100000 and rerun
```
...
typeof(missing) ## what is the `type`?
typeof(vector) ## what is the `type`?
missing_num <- as.numeric(missing) ## turn logical into numeric  
missing_num
sum(missing_num) ## how many TRUEs
sum(missing) ## impleicitly turning  logical into numeric
sum(is.na(vector)) ##  how many NAs
```

## Mathematical operations on a `character` vector
```{r, eval=FALSE}
sum(vector %>% as.numeric(), na.rm = TRUE) ## ERROR sum on character
```
...
Mind that `.` is the default decimal placeholder!  
...
For this workshop we use `tidyverse` a lot
```{r}
